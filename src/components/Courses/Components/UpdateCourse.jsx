...
export default function UpdateCourse() {
  ...
  useEffect(() => {
    try {
      axios(
        `https://unionboard-backend.smitghelani.site/getPerticularCourse/${id}`,
        {
          headers: {
        ...
        values["courseTitle"] = "";
        fieldChanged("courseTitle", response.data.result.CourseTitle);
        fieldChanged("courseCategory", response.data.result.CourseLearning);
        fieldChanged(
          "coursePrerequisite",
          response.data.result.CoursePrerequisite
        );
        fieldChanged("targetAudience", response.data.result.CourseAudience);
        fieldChanged("courseCaption", response.data.result.LectureCaption);

        var cnt = 0;
        for (let i = 0; i < response.data.secDT.length; i++) {
          setSectionField([
            ...sectionField,
            {
              SectionName: response.data.secDT[i],
              lectureNo: response.data.lecData[i],
            },
          ]);
          setSectionField([...sectionField]);
          console.log(sectionField);
          console.log(response.data.secDT[i], response.data.lecData[i]);
          fieldChanged(`sectionName${i}`, response.data.secDT[i]);
          for (let j = 0; j < response.data.lecData[i]; j++) {
            fieldChanged(
              `lectureName${i}${j}`,
              response.data.secData[cnt].VideoName
            );
            fieldChanged(
              `lectureDesc${i}${j}`,
              response.data.secData[cnt].LectureDesc
            );
            fieldChanged(
              `lectureVideo${i}${j}`,
              response.data.secData[cnt].LectureVideo.secure_url
            );
            fieldChanged(
              `lectureResource${i}${j}`,
              response.data.secData[cnt].LectureResourceFile.secure_url
            );
        ...
        fieldChanged("landingPageTitle", response.data.resultMore.CourseTitle);
        fieldChanged(
          "landingPageSubTitle",
          response.data.resultMore.CourseSubTitle
        );
        fieldChanged("landingPageDesc", response.data.resultMore.CourseDesc);
        fieldChanged("courseLanguage", response.data.resultMore.CourseLanguage);
        fieldChanged(
          "courseDifficulty",
          response.data.resultMore.DifficultyLevel
        );
        fieldChanged(
          "landingPageCategory",
          response.data.resultMore.CourseCategory
        );
        fieldChanged(
          "landingPageLearning",
          response.data.resultMore.CourseLearing
        );
        fieldChanged("coursePrice", response.data.resultMore.Pricing);
        fieldChanged("courseCoupon", response.data.resultMore.CouponCode);
        fieldChanged("welcomeMessage", response.data.resultMore.WelcomeMessage);
        fieldChanged("congoMessage", response.data.resultMore.CongoMessage);
        fieldChanged("courseMode", response.data.resultMore.Mode);
        fieldChanged(
          "courseImage",
          response.data.resultMore.CourseImg.secure_url
        );
        fieldChanged(
          "coursePromo",
          response.data.resultMore.CoursePromo.secure_url
        );
      });
    } catch (err) {
      setIsActive(false);
  ...
  const validate = (page) => {
    if (page === 1) {
      if (!values["courseTitle"]) {
        ...
        alert(
          "Target audience for the course is required please make sure it's not empty"
        );
        return false;
      } else {
        return true;
      ...
      for (let i = 0; i < sectionField.length; i++) {
        ...
        for (let j = 0; j < sectionField[i].lectureNo; j++) {
          if (!values[`lectureVideo${i}${j}`]) {
            alert(
              `Lecture Video is missing in ${i + 1}th section and ${
                j + 1
              }th lecture`
            );
            return false;
            ...
            alert(
              `Lecture Name is missing in ${i + 1}th section and ${
                j + 1
              }th lecture`
            );
            return false;
            ...
            alert(
              `Lecture Description is missing in ${i + 1}th section and ${
                j + 1
              }th lecture`
            );
            return false;
    ...
    if (page === 3) {
      if (!values["courseTitle"]) {
        ...
        !values["courseLanguage"] ||
        values["courseLanguage"] === "None"
      ) {
        alert("Please choose language of your course");
        return false;
      } else if (
        !values["courseDifficulty"] ||
        values["courseDifficulty"] === "None"
      ) {
        alert("Please choose Difficulty level of your course");
        return false;
      } else if (
        !values["landingPageCategory"] ||
        values["landingPageCategory"] === "None"
      ) {
        ...
        return false;
      } else if (values["coursePrice"] === undefined) {
    ...
    if (validate(page)) {
      ...
      try {
        coursePage = await axios({
          url: "https://unionboard-backend.smitghelani.site/updateCourseOverview",
          method: "POST",
          Headers: {
            "content-Type": "multipart/form-data",
          },
        ...
        for (let j = 0; j < sectionField[i].lectureNo; j++) {
          ...
          try {
            response = await axios({
              url: "https://unionboard-backend.smitghelani.site/updateSectionContent",
              method: "POST",
              Headers: {
                "content-Type": "multipart/form-data",
              },
  ...
  const addLandingPage = async (e) => {
    if (sectionExist) {
      ...
      formdata.append("CourseTitle", values["landingPageTitle"]);
      formdata.append("CourseSubTitle", values["landingPageSubTitle"]);
      ...
      try {
        landingPage = await axios({
          url: "https://unionboard-backend.smitghelani.site/updateLandingPage",
          method: "POST",
          Headers: {
            "content-Type": "multipart/form-data",
          },
      ...
      <div className="courseOverview">
        ...
        <div className="courseTitlePanel">
          <label className="questionLabel">
            ...
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
          </label>
          <input
        ...
        <div className="coursePrerequisite">
          ...
          <input
            ...
            value={values["coursePrerequisite"]}
            onChange={(e) => {
        ...
        <div className="formButton">
          {page > 1 && (
            <button className="formButtonbBack" onClick={prevPage}>
              Back
            </button>
          )}
          &nbsp;
          {page < 5 && (
            <button className="formButtonNext" onClick={nextPage}>
              Next
            </button>
          )}
          {page === 5 && (
            <button className="formButtonSubmit" onClick={onSubmit}>
              Submit
            </button>
          )}
        </div>
  ...
  const secondForm = () => {
    return (
      <div className="courseSection">
          ...
          <div className="Section">
            {sectionField.length > 1 && (
              <div className="deleteButtonSection">
                ...
                <label htmlFor="deleteSection" className="deleteSection">
                  <i className="fa-solid fa-trash-can"></i>
                </label>
              </div>
            )}
            <div className="courseSectionName">
              <label className="courseSectionNameLabel">
                Section Name
                <i className="requiredIcon fa-solid fa-circle-question">
                  <span class="tooltiptext">Reuired !!</span>
                </i>
              ...
              <input
                className="courseSectionNameInput"
                id={`sectionName${indexPri}`}
                value={values[`sectionName${indexPri}`]}
                onChange={(e) => {
            ...
            {Array(parseInt(singleField.lectureNo))
              .fill(1)
              .map((singleLec, index) => (
                <div className="Lecture">
                  {singleField.lectureNo > 1 && (
                    <div className="deleteButtonLecture">
                      <button
                        ...
                        style={{ display: "none" }}
                      ></button>
                      <label htmlFor="deleteLecture" className="deleteLecture">
                        <i className="fa-solid fa-trash-can"></i>
                      </label>
                    </div>
                  )}
                  <div className="courseVideoUpload">
                    <input
                      type="file"
                      id={`lectureVideo${indexPri}${index}`}
                      onChange={(e) => {
                        fieldChanged(
                          `lectureVideo${indexPri}${index}`,
                          e.target.files[0]
                        );
                        document.getElementById(
                          `videoLecture${indexPri}${index}`
                        ).innerHTML =
                    ...
                    />
                    <label
                      htmlFor={`lectureVideo${indexPri}${index}`}
                      id={`videoLecture${indexPri}${index}`}
                    >
                      <i className="courseVideoUploadIcon fa-solid fa-upload"></i>
                      <label
                        htmlFor={`lectureVideo${indexPri}${index}`}
                  ...
                  <div className="courseLectureName">
                    <label className="courseLectureNameLabel">
                      Lecture Name
                      <i className="requiredIcon fa-solid fa-circle-question">
                        <span class="tooltiptext">Reuired !!</span>
                      </i>
                    ...
                    <input
                      className="courseLectureNameInput"
                      id={`lectureName${indexPri}${index}`}
                      value={values[`lectureName${indexPri}${index}`]}
                      onChange={(e) => {
                        fieldChanged(
                          `lectureName${indexPri}${index}`,
                          e.target.value
                        );
                      }}
                    />
                  ...
                  <div className="courseLectureDesc">
                    <label className="courseLectureDescLabel">
                      Lecture Description
                      <i className="requiredIcon fa-solid fa-circle-question">
                        <span class="tooltiptext">Reuired !!</span>
                      </i>
                    ...
                    <textarea
                      className="courseLectureDescInput"
                      id={`lectureDesc${indexPri}${index}`}
                      value={values[`lectureDesc${indexPri}${index}`]}
                      onChange={(e) => {
                        fieldChanged(
                          `lectureDesc${indexPri}${index}`,
                          e.target.value
                        );
                      }}
                  ...
                  <div className="courseResourceUpload">
                    <label className="courseResourceLabel">
                      Course Resources
                    </label>
                    <input
                      type="file"
                      className="courseResourceUploadInput"
                      id={`lectureResource${indexPri}${index}`}
                      onChange={(e) => {
                        fieldChanged(
                          `lectureResource${indexPri}${index}`,
                          e.target.files[0]
                        );
                        document.getElementById(
                          `lecResource${indexPri}${index}`
                        ).innerHTML =
                          "Selected file: " + e.target.files[0].name;
                        document.getElementById(
                          `lecResource${indexPri}${index}`
                    ...
                    />
                    <label
                      htmlFor={`lectureResource${indexPri}${index}`}
                      id={`lecResource${indexPri}${index}`}
                    >
                      <i className="courseResourceUploadIcon fa-solid fa-plus"></i>
                      Add
                    </label>
                  </div>
                </div>
        ...
        <button className="sectionButton" onClick={handleSectionAdd}>
          Add Section
        </button>
        <div className="formButton">
          {page > 1 && (
    ...
    return (
      <div className="landingPageForm">
        ...
        <div className="landingPageTitle">
          <label className="landingPageTitleLabel">
            Course Title
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
          ...
          <input
            className="landingPageTitleInput"
            id="landingPageTitle"
            value={values["landingPageTitle"]}
            onChange={(e) => {
        ...
        <div className="landingPageSubTitle">
          ...
          <input
            className="landingPageSubTitleInput"
            id="landingPageSubTitle"
            value={values["landingPageSubTitle"]}
            onChange={(e) => {
        ...
        <div className="landingPageDesc">
          <label className="landingPageDescLabel">
            Course Description
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
          ...
          <textarea
            className="landingPageDescInput"
            id="landingPageDesc"
            value={values["landingPageDesc"]}
            onChange={(e) => {
        ...
        <div className="courseLanguage">
          <label className="courseLanguageLabel">
            Language
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
        ...
        <div className="courseDifficulty">
          <label className="courseDifficultyLabel">
            Difficulty Level
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
          ...
          <select
            className="courseDifficultyInput"
            label="Difficulty"
            id="courseDifficulty"
            value={values["courseDifficulty"]}
            onChange={(e) => {
        ...
        <div className="landingPageCategory">
          <label className="landingPageCategoryLabel">
          ...
          <select
            className="landingPageCategoryInput"
            label="Category"
            id="landingPageCategory"
            value={values["landingPageCategory"]}
            onChange={(e) => {
        ...
        <div className="landingPageLearning">
          <label className="landingPageLearningLabel">
            What are the learning for this course?
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
          </label>
          <input
            className="landingPageLearningInput"
            id="landingPageLearning"
            value={values["landingPageLearning"]}
            onChange={(e) => {
              fieldChanged("landingPageLearning", e.target.value);
            }}
          />
        </div>
        <label className="courseImageLabel">
          Course Image
          <i className="requiredIcon fa-solid fa-circle-question">
            <span class="tooltiptext">Reuired !!</span>
          </i>
        ...
        <div className="courseImageUpload">
          <input
            ...
            style={{ display: "none" }}
          />
          <label htmlFor="courseImage" id="cimage">
            <i className="courseImageUploadIcon fa-solid fa-upload"></i>
            <label htmlFor="courseImage" className="resourceLabel">
              Upload Image
            </label>
          </label>
        </div>
        ...
        <div className="coursePromoUpload">
          <input
            ...
            onChange={(e) => {
              ...
              document.getElementById("cpromo").innerHTML =
                "Selected file: " + e.target.files[0].name;
              document.getElementById("cpromo").style.fontSize = "18px";
            }}
            style={{ display: "none" }}
          />
          <label htmlFor="coursePromo" id="cpromo">
            <i className="coursePromoUploadIcon fa-solid fa-upload"></i>
            <label htmlFor="coursePromo" className="resourceLabel">
              Upload Promo Video
            </label>
          </label>
        </div>
        <div className="coursePrice">
          <label className="coursePriceLabel">
            Pricing
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
        ...
        <div className="courseCoupon">
          ...
          <input
            className="courseCouponInput"
            id="courseCoupon"
            value={values["courseCoupon"]}
            onChange={(e) => {
    ...
    return (
      <div className="messagePanel">
        <div className="messagePanelWelcome">
          <label className="messagePanelWelomeLabel">
            Welcome Message
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
          ...
          <textarea
            className="messagePanelWelcomeText"
            id="welcomeMessage"
        ...
        <div className="messagePanelCongo">
          <label className="messagePanelCongoLabel">
            Congretulations Message
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
          </label>
          <textarea
            className="messagePanelCongoText"
            id="congoMessage"
            value={values["congoMessage"]}
            onChange={(e) => {
    ...
    return (
      <div className="courseMode">
        <div className="courseModePanel">
          <label className="courseModeLabel">
            Course Mode
            <i className="requiredIcon fa-solid fa-circle-question">
              <span class="tooltiptext">Reuired !!</span>
            </i>
  ...
  const navigatePages = (direction) => () => {
    ...
    setPage(
      findNextPage(
        direction === "next" ? (validate(page) ? page + 1 : page) : page - 1
      )
    );
  };
  ...
  return (
    ...
    >
      <div className="upload">{executeForm(page)}</div>
    </LoadingOverlay>
  );
}
